(ns aave.main
  "Kotodama WASM entrypoint for the Aave clean-room actor (L4) — Clojure port.
  ...
  facts (`aave.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "ReserveData"   :plural "reservedatas"   :id-prefix "aave_res"
    :fields [:liquidityIndex :currentLiquidityRate :variableBorrowIndex :currentVariableBorrowRate :deficit :lastUpdateTimestamp :liquidationGracePeriodUntil :aTokenAddress :variableDebtTokenAddress :accruedToTreasury :virtualUnderlyingBalance] :required [:liquidityIndex :currentLiquidityRate]
    :coerce {:liquidityIndex :int :currentLiquidityRate :int :variableBorrowIndex :int :currentVariableBorrowRate :int :deficit :int :lastUpdateTimestamp :int :liquidationGracePeriodUntil :int :accruedToTreasury :int :virtualUnderlyingBalance :int} :refs {}}
   {:entity "EModeCategory"   :plural "emodecategories"   :id-prefix "aave_emo"
    :fields [:ltv :liquidationThreshold :liquidationBonus :collateralBitmap :isolated :label :borrowableBitmap :ltvzeroBitmap] :required [:ltv :liquidationThreshold]
    :coerce {:ltv :int :liquidationThreshold :int :liquidationBonus :int :collateralBitmap :int :isolated :bool :borrowableBitmap :int :ltvzeroBitmap :int} :refs {}}
   {:entity "UserConfigurationMap"   :plural "userconfigurationmaps"   :id-prefix "aave_use"
    :fields [:data] :required [:data]
    :coerce {:data :int} :refs {}}
   {:entity "ExecuteBorrowParams"   :plural "executeborrowparamses"   :id-prefix "aave_exe"
    :fields [:asset :user :onBehalfOf :interestRateStrategyAddress :amount :interestRateMode :referralCode :releaseUnderlying :oracle :userEModeCategory] :required [:asset :user]
    :coerce {:amount :int :referralCode :int :releaseUnderlying :bool :userEModeCategory :int} :refs {}}])
...
(defn as-int-kernel
  ...
  of aave.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"aave.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aave-compat" :tier tier :entities entities} 200])
