(ns run-tests
  ...
  test namespaces:
    ...
    aave.main-test  (.cljc) -- portable, listed below
    aave.kotoba-qualification-test  (.clj) -- JVM-only, NOT listed

  The qualification namespace requires kotoba.compiler.core, kotoba.runtime
  and kotoba.wasm-exec to compile src/aave/page_limit.kotoba and execute it on
  ...
  Measured 2026-08-20 -- the two runtimes agree on the shared namespace:
    ...
    clojure -M:test   ->  Ran 8 tests containing 108 assertions
    this runner       ->      6 tests containing 98 assertions
